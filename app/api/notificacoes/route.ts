...
export const GET = withAuth(async (request, { user }) => {
  ...
  // Build query: notifications for this specific user OR for their role OR for everyone (null)
  const sql = countOnly
    ? `SELECT COUNT(*) as count FROM notificacoes
       WHERE lida = 0 AND (usuario_id = ? OR role_destino = ? OR (usuario_id IS NULL AND role_destino IS NULL))`
    : `SELECT * FROM notificacoes
       WHERE (usuario_id = ? OR role_destino = ? OR (usuario_id IS NULL AND role_destino IS NULL))
